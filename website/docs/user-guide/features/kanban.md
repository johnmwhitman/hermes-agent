...
| `kanban.disk_governor_state_path` | unset (disabled) | Optional path to a canonical host disk-governor JSON state. When configured, `inventory.pressure.level` of `GREEN` or `YELLOW` permits new dispatcher claims; `RED`, a missing/unreadable/malformed file, or an unsupported level defers all new dispatcher claims for that tick. Manual operator claims are unchanged, as are reclaim, dependency promotion, and already-running workers. Every profile eligible to run the singleton dispatcher should use the same absolute path. |
