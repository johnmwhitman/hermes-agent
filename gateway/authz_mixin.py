...
def _platform_gate_env(name: str, default: str = "") -> str:
    ...
    ``default`` instead of falling through to ``os.environ``. An empty
    scoped value stays empty (deny), it does not borrow the process env.
    Under multiplex the process env may hold ANOTHER profile's
    first-writer-bridged value (the YAML→env bridges in the Discord/Telegram
    adapters' ``_apply_yaml_config`` are first-writer-wins), so falling
    through would leak profile A's allow-all flag or allowlist into profile B
