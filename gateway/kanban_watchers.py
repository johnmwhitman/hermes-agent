    ...
    async def _kanban_dispatcher_watcher(self) -> None:
        ...
        while self._running:
            try:
                ...
                pids = await _run_dispatch_io(_kbd.reap_worker_zombies)
                if pids:
            ...
            try:
                ...
                if not _kanban_dispatch_allowed():
                    ...
                    # See #49638.
                    if _ad_enabled:
                        await _run_dispatch_io(dispatcher.auto_decompose_tick, _ad_per_tick)
                    results = await _run_dispatch_io(dispatcher.tick_once)
                    any_spawned = _log_spawn_results(results)
                    ready_pending = await _run_dispatch_io(dispatcher.ready_nonempty)
                    expected_idle = bool(results) and all(res is not None and _dispatch_result_is_expected_idle(res) for _, res in results)
                    bad_ticks = bad_ticks + 1 if ready_pending and not any_spawned and not expected_idle else 0
                now = int(time.time())
