...
from gateway.config import (
    Platform, _dict_slot, _normalize_choice, _effective_yaml_platform_config,
    _merge_platform_config_blocks, _normalize_platform_config_block,
)
...
def merge_platform_sections(yaml_cfg: dict, gateway_cfg: Any, gw_data: dict) -> dict:
    ...
    ``gateway.platforms.*`` → ``gateway.<platform>`` → top-level ``platforms.*`` → legacy
    top-level ``<platform>``. Normalize shorthand before precedence is applied. An
    ...
    def merge(source_platforms: Any) -> None:
        ...
        for plat_name, plat_block in source_platforms.items():
            ...
            plat_block = _normalize_platform_config_block(plat_name, plat_block)
            merged = _merge_platform_config_blocks(existing, plat_block)
            if "enabled" in plat_block:
                _dict_slot(merged, "extra")["_enabled_explicit"] = True
            platforms_data[plat_name] = merged

    nested_gateway = gateway_cfg if isinstance(gateway_cfg, dict) else {}
    names = set()
    for source in (nested_gateway.get("platforms"), yaml_cfg.get("platforms")):
        if isinstance(source, dict):
            names.update(str(name) for name in source)
    for source in (nested_gateway, yaml_cfg):
        names.update(name for name, block in source.items()
                     if isinstance(block, dict) and _is_platform_name(name))
    for name in names:
        effective = _effective_yaml_platform_config(yaml_cfg, name)
        if effective:
            merge({name: effective})
...
def platform_section(yaml_cfg: dict, name: str, gateway_platforms: Any) -> tuple:
    ...
    return _effective_yaml_platform_config(yaml_cfg, name), isinstance(yaml_cfg.get(name), dict)
