...
from gateway.kanban_watchers_common import _board_slugs, _positive_int_setting, _resolve_profile_limit_overrides, logger
...
class _KanbanDispatcher:
    ...
    def __init__(self, kb: Any, settings: _DispatcherSettings) -> None:
        ...
        self.last_profile_limit_overrides = None
    ...
    def tick_once_for_board(self, slug: str) -> Optional[object]:
        ...
        overrides = _resolve_profile_limit_overrides(load_config, _kbd()._validate_profile_limit_overrides)
        if overrides != self.last_profile_limit_overrides:
            logger.info("kanban dispatcher: effective profile overrides=%s; base cap=%s", overrides, self.settings.max_in_progress_per_profile)
            self.last_profile_limit_overrides = dict(overrides)
        kwargs["max_in_progress_per_profile_overrides"] = overrides
        try:
