...
def audit(direction: str, peer: str, task_id: str, summary: str) -> None:
    ...
    try:
        ...
        rec = {"ts": time.time(), "direction": direction, "peer": peer, "task_id": task_id, "summary": redact_outbound(str(summary or ""))[:500]}
...
def authorize_for_agent(
    ...
    treated as default.
    """
    identity = A2ASecurityContext.capture().authenticate(auth_header, client_ip)
    if identity is None:
        return None
