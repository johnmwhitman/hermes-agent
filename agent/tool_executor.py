...
class _ToolCallRef:
    ...
    def emit_invalid_arguments(self, agent, result: str) -> None:
        # Both sequential and concurrent batches reuse the pre-dispatch
        # refusal slot. Preserve policy provenance instead of labeling it
        # invalid JSON; no request middleware or tool code ran.
        if isinstance(result, _A2APostureBlock):
            self.emit_post(agent, result, trace=[], status="blocked",
                           error_type="a2a_read_only", error_message=str(result))
            return
        self.emit_post(
            agent, result, trace=[],
...
class _A2APostureBlock(str):
    """Early refusal with a truthful policy-block receipt, not a JSON parse error."""


def _a2a_scope_block(agent, function_name: str) -> Optional[str]:
    """Reject before bridge unwrapping, middleware and inline dispatch."""
    ...
    if posture is not None and function_name not in set(posture.get("allowed_tool_names", ())):
        return _A2APostureBlock(f"'{function_name}' is not available under the A2A mutation posture")
    return None
...
def _parse_tool_call(agent, tool_call, *, flatten_probe: bool = False) -> _ParsedCall:
    ...
    scope_block = None
    parse_error = parse_error or _a2a_scope_block(agent, name)
...
def _append_invalid_arguments_result(agent, messages: list, ref: _ToolCallRef, parse_error: str) -> bool:
    ...
    messages.append(make_tool_result_message(
        ref.name, parse_error, ref.call_id,
        effect_disposition="none" if isinstance(parse_error, _A2APostureBlock) else None))
