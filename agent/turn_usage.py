...
def _record_api_call_without_usage(agent, *, increment: bool = True) -> bool:
    """Persist a real completed request without inventing token or cost data."""
    if increment:
    ...
    try:
        ...
        db.queue_token_counts(
            ...
            cost_status="unknown", cost_source="none", api_call_count=1,
        )
        return True
    except Exception:
        logger.debug("API-call accounting without usage failed", exc_info=True)
        return False
...
def record_response_usage(
    ...
    if not (hasattr(response, 'usage') and response.usage):
        _record_api_call_without_usage(agent, increment=False)
