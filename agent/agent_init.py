...
def _load_tools(agent, enabled_toolsets, disabled_toolsets, a2a_policy=None):
    ...
    if a2a_policy is not None:
        allowed = frozenset(a2a_policy.get("allowed_tool_names", ()))
        agent.tools = [t for t in (agent.tools or []) if t.get("function", {}).get("name") in allowed]
...
def _inject_context_engine_tools(agent, a2a_policy=None):
    ...
    if (
        ...
        for _raw_schema in agent.context_compressor.get_tool_schemas():
            ...
            if _schema is None:
                ...
                continue
            _tname = _schema["name"]
            if a2a_policy is not None and _tname not in a2a_policy.get("allowed_tool_names", ()):
                continue
    ...
    if a2a_policy is not None:
        ...
        if not posture.child_policy_matches_tools(a2a_policy, agent.valid_tool_names):
...
def init_agent(
    ...
    a2a_policy: Optional[Dict[str, Any]] = None,
):
    ...
    if a2a_policy is not None:
        from plugins.platforms.a2a import posture
        a2a_policy = posture.load_child_policy(a2a_policy)
        if not a2a_policy or a2a_policy.get("error"):
    ...
    _load_tools(agent, enabled_toolsets, disabled_toolsets, a2a_policy)
    _init_session_state(
    ...
    if a2a_policy is not None:
        agent._session_init_model_config["a2a_posture_binding"] = dict(a2a_policy.get("binding") or {})
    ...
    if a2a_policy is not None:
        allowed = frozenset(a2a_policy.get("allowed_tool_names", ()))
        agent.tools = [t for t in (agent.tools or []) if t.get("function", {}).get("name") in allowed]
        agent.valid_tool_names.intersection_update(allowed)
    ...
    _inject_context_engine_tools(agent, a2a_policy)
