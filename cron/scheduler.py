...
from agent.delegation_context import (
    ...
    enter_delegated_child_cleared_context, exit_delegated_child_cleared_context)
...
from cron.executions import (
    _TERMINAL_STATES, admit_execution, execution_owned_by, finish_execution, get_execution,
...
class _CronRunScope:
    ...
    def __init__(self, job: dict, job_id: str, execution_id: Optional[str]):
        ...
        self._delegated_child_token = None
    ...
    def enter(self) -> None:
        ...
        self._delegated_child_token = enter_delegated_child_cleared_context()
        if self._delegated_child_token.old_value is True:
            logger.warning("Cron scope cleared a leaked delegated-child context")
    ...
    def exit(self) -> None:
        ...
        if self._delegated_child_token is not None:
            exit_delegated_child_cleared_context(self._delegated_child_token)
        for name in _CRON_DELIVERY_VARS:
...
def _run_with_fire_claim_heartbeat(job: dict, run) -> bool:
    ...
    def _finish_unstarted(error: str) -> None:
        ...
        if not execution_id:
            return
        try:
            finish_execution(execution_id, success=False, error=error, owner_token=job.get("_execution_owner_token"))
        except Exception:
            logger.warning(
...
def _ensure_execution_owner(job: dict, *, source: str) -> bool:
    """Admit once or validate the exact carried credential before any dispatch."""
    execution_id = job.get("execution_id")
    owner_token = job.get("_execution_owner_token")
    if execution_id or owner_token:
        return bool(execution_id and owner_token and execution_owned_by(execution_id, owner_token))
    record, owned, owner_token = admit_execution(
        job["id"], source=source, scheduled_instant=job.get("_scheduled_instant"))
    if not owned:
        return False
    job["execution_id"] = record["id"]
    job["_execution_owner_token"] = owner_token
    return True
...
def run_one_job(
    ...
    if not _ensure_execution_owner(job, source="direct"):
        logger.warning("Cron job %s lost execution ownership before dispatch", job["id"])
        return False
...
def _record_fire_ownership_lost(job_id: str, fire_owner: Optional[str], execution_id: str,
    ...
    if fire_owner is not None and heartbeat_fire_claim(job_id, expected_owner=fire_owner):
        ...
        finish_execution(
            ...
            error="Fire claim ownership lost; stale result was discarded.", owner_token=owner_token)
...
def _finish_interrupted_run(job: dict, execution_id: str, delivery_error: Optional[str]) -> None:
    ...
    finish_execution(
        execution_id, success=False,
        error="Interrupted by gateway shutdown before terminal completion.", owner_token=job.get("_execution_owner_token"))
...
def _finish_completed_run(d: _RunDelivery, fire_owner: Optional[str], execution_id: str) -> bool:
    ...
    if fire_owner is not None and not marked:
        finish_execution(
            execution_id, success=False,
            error="Fire claim ownership lost before terminal completion.", owner_token=job.get("_execution_owner_token"))
        return True
    ...
    finish_execution(
        execution_id, success=d.success, error=d.error, delivery_outcome=delivery_outcome, owner_token=job.get("_execution_owner_token"))
    return True
...
def _run_one_job_body(
    ...
    try:
        ...
        if not claim_dispatch(job["id"]):
            ...
            finish_execution(
                execution_id, success=False,
                error="Dispatch claim rejected; execution was not started.", owner_token=job.get("_execution_owner_token"))
        ...
        external_owner = os.environ.get("_HERMES_CRON_EXTERNAL_WORKER") == execution_id
        if not external_owner and mark_execution_running(
            execution_id, owner_token=job.get("_execution_owner_token")) is None:
        ...
        if _fire_claim_ownership_lost():
            _teardown_deferred()
            _record_fire_ownership_lost(job["id"], fire_owner, execution_id, job.get("_execution_owner_token"))
            return True
        ...
        try:
            finish_execution(
                execution_id, success=False, error=_err_text, delivery_outcome=delivery_outcome, owner_token=job.get("_execution_owner_token"))
        except Exception as record_err:
...
def _launch_external_cron_worker(job: dict) -> bool:
    ...
    if mark_execution_handoff_pending(
        execution_id, owner_token=job.get("_execution_owner_token")) is None:
        raise RuntimeError(
...
def _process_due_job(job: dict, adapters, loop, verbose: bool) -> bool:
    ...
    if not claimed:
        finish_execution(
            job["execution_id"], success=False, error="Fire claim lost; execution was not started.", owner_token=job.get("_execution_owner_token"))
        return True
    ...
    claimed_job["_execution_owner_token"] = job.get("_execution_owner_token")
...
def _submit_with_guard(job: dict, pool: concurrent.futures.ThreadPoolExecutor, process_job):
    ...
    try:
        dispatched_job = dict(job)
        if not _ensure_execution_owner(dispatched_job, source="builtin"):
        ...
        release_running_job(job_id)
        _clear_run_claim_best_effort()
        if dispatched_job.get("execution_id") and dispatched_job.get("_execution_owner_token"):
            try:
                finish_execution(dispatched_job["execution_id"], success=False,
                                 owner_token=dispatched_job["_execution_owner_token"],
                                 error=f"Executor setup failed: {execution_err}")
            except Exception:
                logger.exception("Could not terminalize failed dispatch setup for %s", job_label)
        logger.exception(
    ...
    try:
        ...
        try:
            finish_execution(
                dispatched_job["execution_id"], success=False,
                owner_token=dispatched_job.get("_execution_owner_token"),
                error=f"Executor dispatch failed: {submit_err}")
        except Exception:
            logger.exception("Could not terminalize failed dispatch for %s", job_label)
