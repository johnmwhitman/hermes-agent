...
def create_execution(job_id: str, *, source: str, scheduled_instant: Optional[str] = None) -> Dict[str, Any]:
    ...
    """
    record, _owned, _token = admit_execution(job_id, source=source, scheduled_instant=scheduled_instant)
            ...
            conn.execute(
                """INSERT INTO executions
                   (id, job_id, source, process_id, pid, process_started_at,
                    owner_token, status, claimed_at, scheduled_instant)
                   VALUES (?, ?, ?, ?, ?, ?, ?, 'claimed', ?, ?)""",
                (execution_id, str(job_id), str(source), _PROCESS_ID, pid,
                 _process_start_time(pid), local_token, now, canonical_instant(scheduled_instant)),
            )
...
def set_execution_occurrence(execution_id: str, instant: Optional[str], *,
    ...
    with _transaction() as conn:
        cur = conn.execute(
            ...
            "AND handoff_pending=0 AND process_id=? AND pid=? AND owner_token=?",
            (scheduled_instant(instant), execution_id, _PROCESS_ID, os.getpid(), owner_token),
        )
        if cur.rowcount != 1:
            raise RuntimeError("Cron occurrence could not be bound before dispatch")


def mark_execution_handoff_pending(execution_id: str, *,
                                   owner_token: Optional[str] = None) -> Optional[Dict[str, Any]]:
        ...
        cur = conn.execute(
            """UPDATE executions
               ...
               WHERE id=? AND status='claimed'
                 AND process_id=? AND pid=? AND owner_token=?""",
            (time.time(), execution_id, _PROCESS_ID, os.getpid(), owner_token),
        )
        if cur.rowcount != 1:
            return None
...
def adopt_claimed_execution(execution_id: str, *,
                            owner_token: Optional[str] = None) -> Optional[Dict[str, Any]]:
    ...
    with _transaction() as conn:
        cur = conn.execute(
            """UPDATE executions
               ...
               WHERE id=? AND status='claimed' AND handoff_pending=1 AND owner_token=?""",
            (_PROCESS_ID, pid, process_started_at, now, execution_id, owner_token),
        )
        if cur.rowcount != 1:
            return None
        record = _fetch(conn, execution_id)
