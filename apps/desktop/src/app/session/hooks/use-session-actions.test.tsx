...
describe('createBackendSessionForSend profile routing', () => {
  ...
  // A genuinely empty first-run selection must not pin the session. A concrete
  // model with missing provenance is separately covered as legacy fail-closed.
  it('omits the model when no selection is recorded', async () => {
    const params = await createWith(() => {
      setCurrentModel('')
      setCurrentProvider('')
      setCurrentModelSource('')
  ...
  it('keeps configured fallback for a model seeded from the profile default', async () => {
    ...
    expect(params).not.toHaveProperty('model')
    expect(params).not.toHaveProperty('provider')
    expect(params).not.toHaveProperty('fallback_disabled')
  })
