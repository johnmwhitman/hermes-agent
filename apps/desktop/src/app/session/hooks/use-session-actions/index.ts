...
// provider only ride when the composer source is pinned or legacy-unknown — a default-sourced
...
async function desktopSessionCreateParams(
  ...
  // model. Omit model/provider when the source is explicitly 'default'. Older
  // backends may omit provenance; keep their concrete selection pinned.
  const modelSource = getCurrentModelSource()
  const isPinnedSelection = modelSource !== 'default'

  const selection = {
    effort: $currentReasoningEffort.get().trim(),
    fast: $currentFastMode.get(),
    model: isPinnedSelection ? $currentModel.get().trim() : '',
    provider: isPinnedSelection ? $currentProvider.get().trim() : '',
    source: modelSource
  }
