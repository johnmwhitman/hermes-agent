...
def _profile_platform_ports(profile_home: Path, runtime: Optional[dict]) -> Dict[str, int]:
    ...
    for name in active:
        ...
        if not platform_binds_port(name, dict(extra or {})):
            continue
        raw = extra.get(port_key, default_port) if name == "a2a" else block.get(port_key, extra.get(port_key, default_port))
        try:
            ports[name] = int(raw)
