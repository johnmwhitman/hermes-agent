...
class CurrentConfigReadError(RuntimeError):
    """A security-sensitive current config read failed without stale fallback."""


def _read_current_config_overlay(path: Path, label: str) -> Dict[str, Any]:
    try:
        ...
        raise CurrentConfigReadError(f"{label} config is not currently readable") from exc
    if value is None:
        return {}
    if not isinstance(value, dict):
        raise CurrentConfigReadError(f"{label} config root is not a mapping")
    return value


def load_config_readonly_strict() -> Dict[str, Any]:
    """Fresh defaults/user/managed merge for privacy gates; never reads or updates LKG caches."""
    with _CONFIG_LOCK:
        user = _read_current_config_overlay(get_config_path(), "profile")
        if "max_turns" in user:
            section = dict(user.get("agent") or {})
            if section.get("max_turns") is None:
                section["max_turns"] = user["max_turns"]
            user["agent"] = section
            user.pop("max_turns", None)
        normalized = _canonicalize_config(_deep_merge(copy.deepcopy(DEFAULT_CONFIG), user))
        expanded = _expand_env_vars(normalized)
        try:
            managed_dir = managed_scope.get_managed_dir()
        except Exception as exc:
            raise CurrentConfigReadError("managed config location is not readable") from exc
        managed = _read_current_config_overlay(managed_dir / "config.yaml", "managed") if managed_dir else {}
        if managed:
            overlay = _normalize_root_model_keys(managed)
            if isinstance(overlay.get("model"), str):
                overlay = dict(overlay)
                overlay["model"] = {"default": overlay["model"]}
            expanded = _deep_merge(expanded, _expand_env_vars(overlay))
        return expanded
