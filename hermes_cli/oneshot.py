...
def _fallback_chain_for_selection(cfg: dict, *, model=None, provider=None, env_model="") -> list[dict]:
    """Explicit one-shot route pins never activate the ambient fallback chain."""
    return [] if (model or provider or env_model) else get_fallback_chain(cfg)
...
def _run_agent(
    ...
    try:
        agent = AIAgent(
            ...
            fallback_model=_fallback_chain_for_selection(cfg, model=model, provider=provider,
                env_model=os.getenv("HERMES_INFERENCE_MODEL", "").strip()) or None,
