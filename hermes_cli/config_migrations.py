...
def _migrate_to_39(results: Dict[str, Any], quiet: bool) -> None:
    ...
    for section in ("platform_toolsets", "known_builtin_toolsets"):
        ...
        for platform, toolsets in mapping.items():
            if isinstance(toolsets, list) and any(ts in {"bfl", "stt"} for ts in toolsets if isinstance(ts, str)):
                mapping[platform] = [ts for ts in toolsets if not isinstance(ts, str) or ts not in {"bfl", "stt"}]
                changed = True
        if changed:
    ...
    if changed:
        _commit(
            config, results, quiet,
            "removed retired/config-only bfl and stt saved toolsets",
...
def _migrate_to_43(results: Dict[str, Any], quiet: bool) -> None:
    """Reconcile custom/upstream v40 by stored behavior, never by version identity.

    Both histories used v40. Replay the idempotent saved-toolset cleanup and
    preserve an explicit old TTL's duration when expressing it in minutes.
    Existing minute values win; malformed or unknown values remain untouched.
    No provider/model/memory fields are rewritten by this bridge.
    """
    _migrate_to_39(results, quiet)
    config = read_raw_config()
    catalog = config.get("model_catalog")
    if not isinstance(catalog, dict) or "ttl_minutes" in catalog:
        return
    hours = catalog.get("ttl_hours")
    if isinstance(hours, bool) or not isinstance(hours, (int, float)):
        return
    import math
    if not math.isfinite(hours) or hours <= 0:
        return
    catalog["ttl_minutes"] = hours * 60
    del catalog["ttl_hours"]
    _commit(config, results, quiet, "preserved explicit catalog TTL in minutes",
            "  ✓ Preserved explicit model catalog TTL while reconciling custom v40")
