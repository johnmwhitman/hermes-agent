...
def _surviving_pre_update_serve_runtimes(plan) -> list[dict]:
    ...
    try:
        for runtime in getattr(plan, "runtimes", ()) or ():
            ...
            planned[pid] = {
                ...
                "_process_table": isinstance(detail, dict) and detail.get("inventory_source") == "process-table"}
    ...
    def _still_live(pid, row) -> bool:
        if row["_process_table"] or (live is not None and pid not in live):
            # Legacy/custom Desktop launchers may never register in the ledger.
            # Ledger absence cannot prove any inventoried process stopped:
            # the ledger may be missing or a row may have been lost.
            from hermes_cli.process_identity import _pid_alive_matches

            return _pid_alive_matches(pid, row["_create_time"]) is not False
        if live is None:
            return True
