...
def ledger_entries(*, project_root: Optional[Path] = None, read_only: bool = False) -> list[dict]:
    ...
    Upgrade probes use read_only=True: never quarantine or disguise unreadable
    evidence as an empty roster. The caller records uncertainty and aborts.
    """
    want_install = install_id(project_root)
    with _LEDGER_LOCK:
        entries = (_read_ledger(_ledger_path()) if read_only
                   else _read_ledger_or_quarantine(_ledger_path()))
    if entries is None:
        if read_only:
