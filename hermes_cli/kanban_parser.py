...
_SPECS = [
    _cmd("gave-up-triage", [
        _json_flag(),
        _arg("--retry", dest="retry_ids", nargs="+"),
        _arg("--archive", dest="archive_ids", nargs="+"),
        _arg("--reason", help="Required audit reason with retry/archive"),
        _arg("--failure-limit", type=int),
    ], help="Enumerate or reason-stamp retry/archive of gave-up orphans"),
    ...
    _cmd("create", [
        ...
        _arg("--production-effect", dest="production_effect",
             choices=["production", "enabling", "internal"],
             help="Classify the card's user-facing effect: production (receipt must cite deploy/tag/URL), "
                  "enabling (unblocks a production card), internal (default)."),
        ...
        _arg("--kind", dest="block_kind", choices=sorted(kb.VALID_BLOCK_KINDS), help="Required for initial blocked cards; ignored otherwise."),
    ...
    _cmd("block", [
        _arg("--retype", action="store_true", dest="retype_only", help="Repair an already-blocked card kind without a new blocked transition; requires reason."),
        _TASK_ID,
