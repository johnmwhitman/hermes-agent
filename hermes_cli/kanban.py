...
_DELEGATED_CHILD_DENIED_ACTIONS: frozenset[str] = frozenset({
    ...
    "schedule", "unblock", "promote", "archive", "gave-up-triage", "dispatch", "daemon", "repair",
...
def _cmd_create(args: argparse.Namespace) -> int:
    ...
    with kbc.connect_closing() as conn:
        task_id = kb.create_task(
            ...
            production_effect=getattr(args, "production_effect", None),
...
def _cmd_block(args: argparse.Namespace) -> int:
    ...
    if getattr(args, "retype_only", False):
        if not kind or not reason:
            return _err("kanban: --retype requires --kind and an audit reason", 2)
        with kbc.connect_closing() as conn:
            def retype(tid):
                ok, why = kb.retype_block_kind(conn, tid, kind=kind, reason=reason)
                if ok:
                    kb.add_comment(conn, tid, _profile_author(), f"BLOCK_KIND_RETYPED: {reason} (-> kind={kind})")
                return ok
            return _bulk_apply(_bulk_ids(args), retype, lambda tid: f"Re-typed {tid} block_kind={kind}: {reason}", lambda tid: f"cannot retype {tid} (must already be blocked)")
