...
def _quarantine_xai_oauth_tokens(exc: AuthError) -> None:
    ...
    from hermes_cli.auth import (
        _last_auth_error_marker, _load_auth_store, _load_provider_state_with_source,
        _save_auth_store, _store_provider_state, _auth_file_path, _same_path,
        _persist_provider_state_to_store,
    )
    try:
        store = _load_auth_store()
        state, source = _load_provider_state_with_source(store, "xai-oauth")
        state = state or {}
        ...
        if source is not None and not _same_path(source, _auth_file_path()):
            _persist_provider_state_to_store("xai-oauth", state, source, set_active=False)
        else:
...
def resolve_xai_oauth_runtime_credentials(
    *, force_refresh: bool = False, refresh_if_expiring: bool = True,
    ...
    from hermes_cli.auth import _auth_store_lock, _global_auth_file_path, _is_terminal_xai_oauth_refresh_error, _refresh_xai_oauth_tokens, _xai_oauth_discovery
