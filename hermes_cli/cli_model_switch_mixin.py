...
def _commit_model_switch(
    ...
    old_model = cli.model
    snapshot = (getattr(cli, "_pending_one_turn_model_restore", None) or cli._snapshot_model_runtime()) if one_turn else None
    if not cli._stage_and_swap_model(result, old_model, persist_billing_route=not one_turn):
        return
    cli._set_model_switch_fallback_policy(disabled=not persist_global)
    if not picker:
...
class CLIModelSwitchMixin:
    ...
    def _set_model_switch_fallback_policy(self, *, disabled: bool) -> None:
        ...
        from hermes_cli.config import get_fallback_chain
        if disabled:
            chain = []
        else:
            try:
    ...
    def _stage_and_swap_model(self, result, old_model, *, persist_billing_route=True) -> bool:
        ...
        if self.agent is not None:
            try:
                self.agent.switch_model(
                    ...
                    persist_billing_route=persist_billing_route)
            except Exception as exc:
