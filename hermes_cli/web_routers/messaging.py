...
def _multiplex_port_binding_conflict(platform_id: str, requested_profile: Optional[str]) -> Optional[str]:
    ...
    """
    from gateway.config import PORT_BINDING_PLATFORM_VALUES, load_gateway_config, platform_binds_port
    ...
    platform_extra = next((config.extra for platform, config in target_config.platforms.items()
                           if platform.value == platform_id), None)
