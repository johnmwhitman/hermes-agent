...
def _platform_toolset_summary(config: dict, platforms: Optional[List[str]] = None) -> Dict[str, Set[str]]:
    ...
    return {pkey: _get_platform_tools(config, pkey, strict=False) for pkey in platforms}
...
def _get_platform_tools(config: dict, platform: str, *, include_default_mcp_servers: bool = True, strict: bool = True) -> Set[str]:
    ...
    if explicitly_configured and toolset_names:
        _warn_all_invalid_platform_toolsets(platform, platform_toolsets[platform], strict=strict)
...
def _warn_all_invalid_platform_toolsets(platform: str, explicit: list, *, strict: bool = True) -> None:
    ...
    if strict and named and not any(validate_toolset(t) for t in named):
        from hermes_cli.config import get_config_path
        raise ToolsetConfigError(platform, named, str(get_config_path()))
...
def _current_platform_tools(config: dict, pkey: str) -> Set[str]:
    return _get_platform_tools(config, pkey, include_default_mcp_servers=False, strict=False)
