...
@pytest.mark.parametrize("alive, expected", [(True, [4242]), (None, [4242]), (False, [])])
@pytest.mark.parametrize("source", ["process-table", "ledger"])
def test_process_table_backend_is_not_cleared_by_empty_ledger(monkeypatch, alive, expected, source):
    """Custom ephemeral backends need OS liveness proof, not ledger absence."""
    runtime = _serve_runtime(4242)
    runtime.detail["inventory_source"] = source
    monkeypatch.setattr(_identity_module(), "ledger_entries", lambda **kwargs: [])
    monkeypatch.setattr(_identity_module(), "_pid_alive_matches", lambda pid, created: alive)

    rows = abort_recovery._surviving_pre_update_serve_runtimes(_plan(runtime))

    assert [row["pid"] for row in rows] == expected
    assert all("_process_table" not in row for row in rows)
