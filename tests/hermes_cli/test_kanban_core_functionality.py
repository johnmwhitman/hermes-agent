...
@pytest.mark.parametrize("violation_limit", [2, 3])
def test_protocol_violation_budget_not_consumed_by_other_failures(
    kanban_home, monkeypatch, violation_limit,
):
    ...
    import hermes_cli.kanban_db as _kb
    from hermes_cli import kanban_db_dispatch as _kbd
    monkeypatch.setattr(_kbd, "_PROTOCOL_VIOLATION_FAILURE_LIMIT", violation_limit)
    conn = kbc.connect()
    try:
        ...
        # Both the production limit (2) and upstream limit (3) must permit
        # exactly limit - 1 violation retries, without consuming the unified
        # crash budget or resetting the streak into an infinite retry loop.
        for i, pid in enumerate(range(991001, 991000 + violation_limit)):
            ...
            assert _kbd._protocol_violation_streak(conn, tid) == i + 1

        # The configured consecutive violation bound must still block.
        _drive_protocol_violation(conn, tid, 991000 + violation_limit)
        task = kb.get_task(conn, tid)
