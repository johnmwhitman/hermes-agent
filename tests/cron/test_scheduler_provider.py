...
def test_claim_fire_persists_attempt_before_fire_claimed(monkeypatch):
    ...
    events = []
    real_admit = executions.admit_execution
    monkeypatch.setattr(
        jobs,
        "claim_job_for_fire",
    ...
    monkeypatch.setattr(
        executions,
        "admit_execution",
        lambda jid, source, **_kw: events.append("ledger")
        or real_admit(jid, source=source, **_kw),
    )
    monkeypatch.setattr(
        sched,
        "run_one_job",
...
def test_fire_due_forwards_manual_force_to_store_claim(monkeypatch):
    ...
    def run_and_finish(job, **kw):
        assert executions.finish_execution(
            job["execution_id"], success=True,
            owner_token=job["_execution_owner_token"],
        ) is not None
        return True

    monkeypatch.setattr(sched, "run_one_job", run_and_finish)
