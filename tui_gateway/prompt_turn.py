...
def _prepare_turn_input(sid: str, session: dict, st: _TurnRun, text: Any, images: list[str]):
    ...
    _wire_callbacks(sid)
    st.one_turn_restore = _prepare_turn_model_runtime(sid, session)
...
def _finish_turn(sid: str, session: dict, st: _TurnRun) -> None:
    ...
    if st.one_turn_restore:
        try:
            ...
            session["blocked_one_turn_model_restore"] = copy.deepcopy(st.one_turn_restore)
            _emit("error", sid, {"message": "One-turn model restore failed; future prompts are blocked until the original runtime is restored."})
...
def _run_prompt_submit(
    ...
    def run():
        ...
        st = _TurnRun(
            session["agent"], None, terminal_callback,
