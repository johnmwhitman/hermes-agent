...
def _commit_agent_switch(sid: str, session: dict, agent, result, current_model: str, snapshot,
                         *, fail_closed_pin: bool, pin_session_override: bool):
    ...
    try:
        agent.switch_model(
            ...
            persist_billing_route=snapshot is None)
    except Exception as exc:
    ...
    if snapshot is None and pin_session_override:
...
def _apply_model_switch(
    ...
    if one_turn and agent:
        existing_restore = session.get("one_turn_model_restore")
        restore_snapshot = (copy.deepcopy(existing_restore) if isinstance(existing_restore, dict)
                            else _snapshot_agent_model_runtime(agent))
    ...
    if agent:
        _commit_agent_switch(sid, session, agent, result, current_model, restore_snapshot,
                             fail_closed_pin=fail_closed_pin, pin_session_override=pin_session_override)
