...
def _stored_session_runtime_overrides(row: dict | None) -> dict:
    ...
    if model:
        ...
        overrides["model_override"] = {
            ...
            "fallback_disabled": model_config.get("fallback_disabled") is not False}
    if provider:
...
def _persist_live_session_runtime(session: dict | None) -> None:
    ...
    try:
        ...
        if isinstance(model_override, dict) and "fallback_disabled" in model_override:
            model_config["fallback_disabled"] = model_override["fallback_disabled"] is not False
        else:
...
def _session_info(agent, session: dict | None = None) -> dict:
    ...
    model_override = sess.get("model_override")
    if isinstance(model_override, dict) and str(model_override.get("model") or "").strip():
        info["fallback_disabled"] = model_override.get("fallback_disabled") is not False
    with contextlib.suppress(Exception):
...
def _resolve_runtime_with_fallback(resolve_kwargs: dict | None = None, *,
                                   allow_fallback: bool = True) -> _RuntimeFallbackResolution:
...
def _resolve_agent_model_runtime(model_override, provider_override) -> tuple[str, dict]:
    ...
    if isinstance(model_override, dict) and model_override.get("model"):
        ...
        overrides = {}
    fallback_disabled = bool(isinstance(model_override, dict) and model_override.get("model")
                             and model_override.get("fallback_disabled") is not False)
    resolution = _resolve_runtime_with_fallback(resolve_kwargs, allow_fallback=not fallback_disabled)
...
def _make_agent(
    ...
    agent = AIAgent(
        ...
        fallback_model=[] if fallback_disabled else _load_fallback_model(),
        **_agent_cbs(sid))
...
def _lazy_resume_info(cwd: str, *, model: str = "", provider: str = "", profile: str | None = None,
                      fallback_disabled: object = False) -> dict:
...
def _fallback_session_info(session: dict) -> dict:
    ...
    return _lazy_resume_info(
        cwd, model=override.get("model") or "", provider=override.get("provider") or "",
        fallback_disabled=override.get("fallback_disabled") is not False if override.get("model") else False,
        profile=profile_name_for_home(session.get("profile_home")))
