...
def _(rid, params: dict) -> dict:
    ...
    return _ok(rid, {
        ...
        "info": {"model": override.get("model") if override else _resolve_model(),
                 "fallback_disabled": override.get("fallback_disabled") is not False if override else False,
...
class _Resume:
    ...
    def info(self, cwd: str, overrides: dict) -> dict:
        return _lazy_resume_info(cwd, model=(overrides.get("model_override") or {}).get("model") or "",
                                 fallback_disabled=(overrides.get("model_override") or {}).get("fallback_disabled", False),
...
def _resume_live_unpersisted(ctx: _Resume, live_sid: str, live: dict) -> dict:
    ...
    return _ok(ctx.rid, _attach_todo_state({
        ...
        "info": {**_fallback_session_info(live), "profile_name": profile_name_for_home(live.get("profile_home")) or _response_profile_name(ctx.profile)}}, live))
