...
from tools.mcp_tool_input_required import _call_with_input_required
...
async def _call_tool_racing_stdio_death(server, server_name: str, tool_name: str, args: dict):
    ...
    _call_coro = _call_with_input_required(server, server.session.call_tool, tool_name, arguments=args)
...
def _make_utility_handler(op: str, log_label: str, rpc, render, required: Optional[str] = None):
    """``(server_name, tool_timeout) -> sync handler`` for one utility tool: ``rpc(server, args,
        ...
        def _handler(args: dict, **kwargs) -> str:
            ...
            async def _call():
                async with server._rpc_lock:
                    ...
                    try:
                        result = await rpc(server, args, server_name)
                    finally:
                        server._pending_call_context = None
...
_make_list_resources_handler = _make_utility_handler(
    ...
    lambda server, args, sn: _core._paginate_full_list(server.session.list_resources, "resources", sn), _render_resource_list)
_make_read_resource_handler = _make_utility_handler(
    "resources/read", "read_resource",
    lambda server, args, sn: _call_with_input_required(server, server.session.read_resource, args["uri"]), _render_read_resource, required="uri")
_make_list_prompts_handler = _make_utility_handler(
    "prompts/list", "list_prompts",
    lambda server, args, sn: _core._paginate_full_list(server.session.list_prompts, "prompts", sn), _render_prompt_list)
_make_get_prompt_handler = _make_utility_handler(
    "prompts/get", "get_prompt",
    lambda server, args, sn: _call_with_input_required(server, server.session.get_prompt, args["name"], arguments=args.get("arguments", {})),
