...
from tools.kanban_tools_schemas import (
    KANBAN_ARCHIVE_SCHEMA, KANBAN_REASSIGN_SCHEMA,
...
def _enforce_worker_task_ownership(tid: str) -> None:
    ...
    if env_tid and tid != env_tid and _triage_actor_profile() is None:
        raise _Reject(
...
def _require_orchestrator_tool(tool_name: str) -> None:
    ...
    if os.environ.get("HERMES_KANBAN_TASK") and _triage_actor_profile() is None:
        raise _Reject(
...
def _handle_complete(args: dict, **kw) -> str:
    ...
    with _board(args.get("board")) as (kb, conn):
        ...
        receipt_err = _enforce_receipt_on_complete(
            {**args, "result": result, "summary": summary, "metadata": metadata},
            conn, tid, body=task.body if task else "",
        )
        if receipt_err:
            return receipt_err
...
def _handle_archive(args: dict, **kw) -> str:
    tid = _worker_guard("kanban_archive", args)
    reason = _redact_opt(args.get("reason"))
    _check(reason and reason.strip(), "reason is required for an attributable archive")
    with _board(args.get("board")) as (kb, conn):
        error = _enforce_triage_ownership(kb, conn, "kanban_archive", tid)
        if error:
            return error
        _check(kb.archive_task(conn, tid), f"could not archive {tid}")
        with kb.write_txn(conn):
            kb._append_event(conn, tid, "archive_reason", {"reason": reason, "actor": os.environ.get("HERMES_PROFILE")})
        return _ok(task_id=tid, status="archived")

@_kanban_handler("kanban_reassign")
def _handle_reassign(args: dict, **kw) -> str:
    tid = _worker_guard("kanban_reassign", args)
    raw_assignee = args.get("assignee")
    assignee = str(raw_assignee).strip() if raw_assignee is not None else ""
    if assignee.lower() in {"none", "-", "null"}:
        assignee = ""
    _check(assignee, "assignee is required")
    reason = _redact_opt(args.get("reason"))
    _check(reason and reason.strip(), "reason is required for an attributable reassign")
    reclaim = _parse_bool_arg(args, "reclaim_first")
    with _board(args.get("board")) as (kb, conn):
        error = _enforce_triage_ownership(kb, conn, "kanban_reassign", tid)
        if error:
            return error
        prior = _existing_task(kb, conn, tid)
        _check(kb.reassign_task(conn, tid, assignee, reclaim_first=reclaim, reason=reason), f"could not reassign {tid}; a live run requires reclaim_first=true")
        with kb.write_txn(conn):
            kb._append_event(conn, tid, "reassign_reason", {"reason": reason, "from": prior.assignee, "to": assignee, "actor": os.environ.get("HERMES_PROFILE")})
        return _ok(task_id=tid, assignee=assignee, status=kb.get_task(conn, tid).status)
