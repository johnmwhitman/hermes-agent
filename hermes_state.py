...
def _ensure_test_isolation(db_path: Path) -> None:
    ...
    try:
        ...
        return
    from hermes_state_guard import _logical_platform_state_root
    roots = []
    for extra in (_logical_platform_state_root(), *_STATE_DB_GUARD_EXTRA_DENY_ROOTS):
        if extra is None:
            continue
        try:
            raw = Path(os.path.abspath(str(Path(extra).expanduser())))
            roots.extend((raw, raw.resolve()))
        except Exception:
            continue
    for root in roots:
        if any(_is_production_state_db(candidate, root) for candidate in (lexical, resolved)):
            raise RuntimeError(
