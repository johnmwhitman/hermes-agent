...
class AIAgent(
    ...
    def __init__(
        ...
        a2a_policy: Dict[str, Any] | None = None,
    ):
    ...
    def _ensure_db_session(self) -> None:
        ...
        try:
            ...
            create_config = self._session_row_model_config()
            runtime = getattr(self, "_session_create_runtime_override", None)
            if isinstance(runtime, dict):
                create_model = runtime.get("model") or create_model
                create_config = dict(create_config or {})
                for key in ("model", "provider", "base_url", "api_mode"):
                    if runtime.get(key):
                        create_config[key] = runtime[key]
                    else:
                        create_config.pop(key, None)
            self._session_db.create_session(
                session_id=self.session_id, source=source, model=create_model,
                model_config=create_config, system_prompt=self._cached_system_prompt,
